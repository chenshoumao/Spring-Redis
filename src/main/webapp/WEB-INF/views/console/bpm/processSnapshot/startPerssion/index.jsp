...
<script type="text/javascript">
	...
	function saveAlreadSelectRoleData() {
		...
		var id = "owners_roleName_value";
		$("#" + id).val(name);
		$("#" + id + "_num").val(value);
	}

	function opneRoleDlg() {
	...
	function opneDeptDlg() {
		...
		var deptDlg = layer.open({
			...
			yes : function(index, layero) {
				...
				$("#owners_deptId_value_num").val(ids);
				$("#owners_deptId_value").val(name);
				layer.close(deptDlg);
			},
			btn2 : function(index, layero) {
			}
	...
	$.ajax({
		url : "${pageContext.request.contextPath}/console/startPermissions/saveOrUpdate.action",
		type : "post",
		data : {
			"snapshotId" : "${snapshotId}",
			"people" : $("#owners_people_value_num").val(),
			"deptId" : $("#owners_deptId_value_num").val(),
			"roleId" : $("#owners_roleName_value_num").val()
		},
		dataType : "json",
		success : function(result) {
			layer.close(loadTip);
...
<body>
	...
	<div id="myTabContent" class="tab-content">
		...
		<p class="flri" style="margin-top: 15px; margin-right: 20%">
			<button id="saveProConfig" onclick="saveProConfig();">保存</button>
		</p>
	</div>
